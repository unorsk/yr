...
import WeatherModel (Forecast(..), WeatherSymbol(..), Temperature(..), ShortInterval(..), DayInterval(..))
...
doForecast arg =
  let forecastPrintingF = case extended arg of
        ...
        Extended -> printForecastExetended in
          do
...
printForecast forecast =
  let interval = take 1 forecast.shortIntervals in
    concatMap (\i -> show (i.temperature.value) <> "Сº  " <> show i.precipitation <> "  " <> show i.symbolCode) interval
...
fetchForecast city = do
  ...
  pure $ case res of
    ...
    Right response -> case getResponseBody response of
      -- TODO BETTER ERROR HANDLING
      Left _e -> Left $ "Couldn't parse response. Response code: " <> show (getResponseStatusCode response) <> show _e
